...
llms.txt 유무는 실측한 값이다. 사이트가 나중에 추가할 수 있으니 없다고 적힌 곳도 한 번 더 확인해볼 만하다 (`curl -sI <site>/llms.txt` — SPA는 200에 `text/html`을 반환하므로 content-type까지 볼 것).
...
응용: 문서 컴포넌트가 registry JSON의 의존성 목록을 읽어 코드 탭을 자동 생성하게 하면, 예제와 실제 배포물이 어긋날 여지가 사라진다.
...
**볼 것**: **MCP 서버를 문서 전달 채널로 제공**한다(`apps/mcp/`). llms.txt가 정적 텍스트 덤프라면 MCP는 질의 가능한 인터페이스다. 에이전트 대상 문서 제공의 두 번째 형태다. llms.txt 다음 단계를 고민한다면 여기가 참고 지점.
...
반대쪽 극단은 컴포넌트 페이지 하나에 가이드라인과 API를 같이 두는 방식이다. 어느 쪽이 맞는지는 독자가 갈리는 정도와 갱신 주기가 결정한다.
...
**주의**: 레포 활동이 느린 편이다. 코드보다 문서 참고용으로 볼 것.

---

## 문서 구조를 짤 때 챙길 축 (위 사례들에서 공통으로 갈리는 지점)

1. **가이드라인과 API를 한 페이지에 둘 것인가, 분리할 것인가** — Spectrum은 사이트 자체를 나눴고, Polaris는 한 페이지에 둔다. 독자가 디자이너와 개발자로 갈릴수록 분리가 유리하다
2. **코드 예제의 출처** — 문서에 하드코딩할지, registry/소스에서 끌어올지. shadcn은 후자라 문서와 배포물이 어긋날 여지가 구조적으로 없다
...
5. **플랫폼 축이 IA에 들어가는가** — 웹/네이티브처럼 대상이 여럿이면 피할 수 없는 계층이고, 단일 플랫폼이면 불필요한 깊이만 늘린다
