...
- **이미 저장되어 있는가?** — 대화에서 mind-palace 또는 obsidian-design-system-knowledge 스킬이 호출된 적이 있는지 확인한다. 호출이 있었다면, 그 스킬이 저장한 내용의 제목이나 핵심 키워드와 겹치는 항목은 제외한다. 확인 방법: 대화에서 `Skill` 도구 호출 중 `mind-palace` 또는 `obsidian-design-system-knowledge`를 인자로 사용한 기록이 있는지, 그리고 그 호출 결과로 Obsidian에 저장된 노트 제목이 무엇이었는지를 역추적한다.
